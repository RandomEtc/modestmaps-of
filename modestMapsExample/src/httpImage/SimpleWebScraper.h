...
class SimpleWebScraper : public ofThread  {

    public:

        SimpleWebScraper();
	
		// empty copy constructor and assignment operator because the default was breaking with BasicEvent
		SimpleWebScraper(const SimpleWebScraper& swc) {
		}
		SimpleWebScraper& operator=(const SimpleWebScraper& swc) {
			return *this;
		}
